            ...
            rl.question('Type an option: ', (option) => {
                switch (option) {
                    ...
                    case "11": 
                        ...
                        db.collection('companies').find({ name: 'Facebook' }, { relationships: 1, _id: 0 }).toArray((error, result) => {
                            if (error) {
                                ...
                                result[0].relationships.forEach((elem) => {
                        ...
                        db.collection('companies').find({ name: 'Facebook' }, { relationships: 1, _id: 0 }).toArray((error, result) => {
                            if (error) {
                                ...
                                let arrResult= []
                                result[0].relationships.forEach((elem) => {
                                    if (elem.is_past === true) {
                                        arrResult.push(elem.person.first_name)
                                    }
                                })
                    ...
                    case "13":
                        ...
                        break;
                    case "14": //ESTA MAL, LO SE
                            db.collection('companies').find({"category_code": "social"}, { name: 1, _id: 0 }).toArray((error, result) => {
                                if (error) {
                                    console.log(error);
                                    rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
                                } else {
